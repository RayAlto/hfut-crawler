...
class JxglMobile:
    ...
    def login(self) -> bool:
        ...
        login_response = self.__requests_session.post(
            url=f'{self.index_url}appLogin/login.action',
            data={
        ...
        project_data_response = self.__requests_session.post(
            url=f'{self.index_url}publicdata/getProjectInfo.action',
            data={
        ...
        semester_week_response = self.__requests_session.post(
            url=f'{self.index_url}publicdata/getSemesterAndWeekList.action',
            data={
    ...
    def get_week_schedule(self, week_index: int = None, semester_code: str = None) -> list:
        self.check_login_status()
        week_schedule_response = self.__requests_session.post(
            url=f'{self.index_url}schedule/getWeekSchedule.action',
            data={
                'userKey': self.__user_key,
                'identity': '0',
                'projectId': self.__project_id,
    ...
    def get_classmates(self, lesson_code: str, semester_code: str = None) -> list:
        self.check_login_status()
        classmates_response = self.__requests_session.post(
            url=f'{self.index_url}schedule/getClassList.action',
            data={
    ...
    def get_exam_arrange(self, semester_code: str = None) -> list:
        exam_response = self.__requests_session.post(
            url=f'{self.index_url}course/getExamArrangement.action',
            data={
                'projectId': self.__project_id,
                'semestercode': semester_code if semester_code else self.__semester_code,
                'userKey': self.__user_key,
    ...
    def get_score(self, semester_code: str = None) -> dict:
        score_response = self.__requests_session.post(
            url=f'{self.index_url}course/getSemesterScoreList.action',
            data={
                'projectId': self.__project_id,
